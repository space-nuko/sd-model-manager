...
from sd_model_manager.models.sd_models import PreviewImage, SDModel, LoRAModel, LoRAModelSchema
...
async def show(request):
    ...
    async with request.app["db"].AsyncSession() as s:
        ...
        if row is None:
            return web.json_response({"message": f"LoRA not found: {model_id}"}, status=404)
        row = row[0]
        ...
        if "preview_images" in changes:
            ...
            for image in changes["preview_images"]:
                if "id" in image:
                    ...
                    if existing is None:
                        return web.json_response({"message": f"Preview image not found: {image['id']}"}, status=404)
