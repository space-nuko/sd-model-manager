...
class PreviewGeneratorDialog(wx.Dialog):
    def __init__(self, parent, app, items):
        ...
        self.sdmm_api = ModelManagerAPI(self.app.config)
    ...
    def save_preview_image(self, item, result):
        ...
        result = self.sdmm_api.update_lora_sync(item["id"], changes)
    ...
    def OnOK(self, evt):
        ...
        if self.result is not None:
            self.save_preview_image(self.items[0], self.result)
    ...
    def OnUpscale(self, evt):
        utils.start_thread(self.upscale_prompt, self.last_data)

    def start_prompt(self, data=None):
        utils.start_thread(self.run_prompt, data)

    def run_prompt(self, data):
        try:
            ...
            wx.CallAfter(self.on_fail, ex)

    def upscale_prompt(self, data):
        try:
...
async def run(app, items):
    ...
    dialog.Center()
    result = await wxasync.AsyncDialogShowModal(dialog)
