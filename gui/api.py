...
class ModelManagerAPI:
    ...
    def update_lora_sync(self, id, changes):
        ...
        """
        req = urllib.request.Request(self.base_url() + f"/api/v1/lora/{id}", method="PATCH")
...
class ComfyAPI:
    ...
    def get_filepaths(self, folder):
        ...
        with urllib.request.urlopen(f"http://{self.server_address}/filepaths?{query}") as response:
    ...
    def get_relative_path(self, folder, full_path):
        ...
        query = urllib.parse.urlencode(params)
        with urllib.request.urlopen(f"http://{self.server_address}/relative_path?{query}") as response:
            return simplejson.loads(response.read())
